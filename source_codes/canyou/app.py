...
def login():
    ...
    if request.method == "GET":
        ...
        if user:
            print(user)
            if user[0] == 'admin':
                return render_template("dashboard.html", flag="flag{w0rld_0f_sQl_8kdw7}")
            else:
                return render_template("dashboard.html", flag="flag{f1l7ers_n0t_s3cur3}")
        else:
            return render_template("index.html", error="Wrong username or password")    

if __name__ == '__main__':
    app.run(host="0.0.0.0", port=1596, debug=True)
